...
function! s:GetSongInfo()
	let artist = system('osascript -e ''tell application "Spotify" to artist of current track''')
	let album = system('osascript -e ''tell application "Spotify" to album of current track''')
	let song = system('osascript -e ''tell application "Spotify" to name of current track''')

	let artist = Trim_whitespace(artist)
	let album = Trim_whitespace(album)
	let song = Trim_whitespace(song)

	echo "Playing ". song . " - " . album . " by " . artist
endfunction
...
command!            SpInfo    call s:GetSongInfo()
